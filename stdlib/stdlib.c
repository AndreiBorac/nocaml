...
WOMBAT_BUILTIN static uintptr_t* wombat_builtin_int_minus_min(WombatExternal* wombat_external, uintptr_t* wombat_context WOMBAT_UNUSED, uintptr_t* cella, uintptr_t* cellb, uintptr_t* cellc)
{
  STDLIB_CHECK_TOP(cellc);
  if ((cella[0] == WOMBAT_NATIVE_CONSTRUCTOR_Integer) && (cellb[0] == WOMBAT_NATIVE_CONSTRUCTOR_Integer) && (cellc[0] == WOMBAT_NATIVE_CONSTRUCTOR_Integer)) {
    cellc[1] = (cella[1] < cellb[1] ? cella[1] : cellb[1]);
    return cellc;
  } else {
    wombat_panic(wombat_external, NULL);
  }
...
WOMBAT_BUILTIN static uintptr_t* wombat_builtin_int_minus_max(WombatExternal* wombat_external, uintptr_t* wombat_context WOMBAT_UNUSED, uintptr_t* cella, uintptr_t* cellb, uintptr_t* cellc)
{
  STDLIB_CHECK_TOP(cellc);
  if ((cella[0] == WOMBAT_NATIVE_CONSTRUCTOR_Integer) && (cellb[0] == WOMBAT_NATIVE_CONSTRUCTOR_Integer) && (cellc[0] == WOMBAT_NATIVE_CONSTRUCTOR_Integer)) {
    cellc[1] = (cella[1] > cellb[1] ? cella[1] : cellb[1]);
